...
Orchestrate the build of a **whole feature**: its reviewed spec (`docs/work/02-development/F<id>_<slug>.md`) plus the written-out issues `/split` produced (`docs/work/02-development/I<id>_<slug>.md`, `parent: F<id>`). You are the **orchestrator** — you drive the issues to done; you do **not** implement them yourself.
