...
- **Deepening an existing codebase** — [improve-codebase.md](improve-codebase.md). The deliberate review: scan for friction, present deepening candidates as a visual report, then grill through the one the user picks (report scaffold: [improve-codebase-report.md](improve-codebase-report.md)). Picking a candidate _generates an idea_ you can take into the main flow.

## How it's reached

`architect` is model-invoked, so other skills activate it mid-work — `/tdd` reaches for the codebase-design aspect at a design decision; `/review-feature` draws on the same vocabulary for its architecture concerns. You can also invoke it by hand for deliberate architectural work — most often the deepening review.
