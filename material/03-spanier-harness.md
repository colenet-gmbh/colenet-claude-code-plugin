...
**Beitrag zur Synthese (Kurzform):** Aus cape-Sicht die **Engineering-Substanz & Bodenhaftung**: das Muster, ein Harness in einer realen Codebase verlässlich zu betreiben. Die *Mechanik* ist Gold und generisch — der *Inhalt* ist Payload/Next.js-spezifisch und muss neu geschrieben werden.
...
| `requirement-engineer` | Feature-Specs (Gherkin-AC, Datenmodell, Abgrenzung); ruft Security + Architekt als Parallel-Review-Subagenten | ✅ Muster (Rolle + Parallel-Review) generisch; Fachfokus (öffentliche Verwaltung/a11y) cape-spezifisch anpassen. |
...
| `tooling/feature-workflow/kanbanize.mjs` | Generischer Businessmap/Kanbanize-API-Wrapper (ENV-konfiguriert, keine hartkodierten Werte, umlautsicherer JSON-Body) | 🔶 Tracker-Anbindung generisch gebaut; nur relevant, wenn cape an einen Tracker spiegeln soll. Vgl. Fabians `ticketing`/`mirror-block`. |
...
| `.gitignore`-Split: `.claude/skills/` geteilt, `settings.local.json` lokal | Team-Wissen wird versioniert, maschinenlokale Hook-Verdrahtung nicht | 🔶 Gutes Modell — cape sollte die **Lücke schließen** (Hook-Verdrahtung doch teilbar via versionierter `settings.json`). |
...
| Kein versioniertes `CLAUDE.md` | Projektgedächtnis bewusst lokal/privat; Wissen über 6 Skills verstreut | — Schwäche: cape sollte ein zentrales, geteiltes Projektgedächtnis vorsehen. |
| Keine `.claude/commands/`, keine `.claude/agents/` | Rollen ad-hoc über Skill-Aufrufe + Subagent-Präambeln, nicht über deklarierte Agents | — Designentscheidung; cape kann hier von Fabians Command→Agent-Muster ergänzen. |
...
- **Wichtig:** Bei Spanier ist fast durchgängig die **Mechanik generisch, der Inhalt app-spezifisch**. cape übernimmt die Muster und schreibt die Rollen-/Hook-Inhalte stack-neutral (oder stack-konfigurierbar) neu.
- **Draußen (⛔):** Payload/Block-spezifische Skripte & Hooks, leeres `.ai/`.

**Offene Knackpunkte für die Auswahl:**

1. Rollen-Modell: Übernimmt cape das Orchestrator+Rollen-Muster — und mit welchen Rollen (stack-neutral, oder konfigurierbare „Stack-Experten-Rolle")?
2. Guardrails: Liefert cape ein generisches 3-Schicht-Guardrail-Kit (Hook-Templates + Git-Hook + CI-Snippets) als eigenes Feature?
3. Feature-Workflow: Übernimmt cape das `docs/features/`-Wandernde-Datei-Kit (konvergiert mit Fabians durable Artefakten → eine gemeinsame Lösung)?
4. Schließt cape Spaniers Lücken bewusst: zentrales geteiltes Projektgedächtnis + teilbare Hook-Verdrahtung?
